...
for /f "skip=1 tokens=1" %%D in ('adb devices') do (
    if not "%%D"=="" if not "%%D"=="*" (
        ...
        if errorlevel 1 (
            echo [FAIL] Could not install on %%D
        ) else (
            echo [OK]   Installed successfully on %%D. Launching app...
...
echo ============================================
echo   Done! All connected devices updated & launched.
echo ============================================
pause
